...
const jsonLock = JSON.parse(
	fs.readFileSync(path.join(__dirname, "..", "package-lock.json")).toString(),
);

// Set version to TS Version
let versionSuffix = (process.argv[2] || "").trim();

const baseVersion = jsonLock["dependencies"]["typescript"]["version"].replace(
	/0?\-\w*\./g,
	"",
);
